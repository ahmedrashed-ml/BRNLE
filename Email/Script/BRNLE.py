...
def EvalMatrix3(UGPredMatrix,UGTrueMatrix):
  ...
  for i in range(USER_NUM):
    ...
    #print(' group  ',i, ' topn ',TopN)
      ...
      TotalUsersAUC=TotalUsersAUC+roc_auc_score(RawTrue,ModPredicted)


  TotalAvgUsersAUC=TotalUsersAUC/ValidUsersCount
...
########
  ...
  TrueLableList=list()
  TotalCorrectHit=0      
  for i in range(UGPredMatrixNewMod.shape[0]):
    ModPredicted=np.array(UGPredMatrixNewMod[i], copy=True) 
    RawTrue=np.array(UGPredMatrixNewTrue[i], copy=True) 
    PredLabel=np.argmax(ModPredicted)
    TrueLabel=np.argmax(RawTrue)
    PredLableList.append(PredLabel)
    TrueLableList.append(TrueLabel)
    if(np.array_equal(ModPredicted,RawTrue)):
      TotalCorrectHit=TotalCorrectHit+1
  print('Correct Hits = ',TotalCorrectHit,  'Total Test Users = ',UGPredMatrixNewMod.shape[0])
...
############
  ...
  for i in range(ITEM_NUM):
    ...
    FN=0
    F1=0
      #print(np.sum(ModPredicted))
      #print(np.sum(RawTrue))
    for j in range(len(ModPredicted)):
      ...
      if(ModPredicted[j]==0 and  ModPredicted[j]!=RawTrue[j]):
        ...
        TotalFN=TotalFN+1
    #print(TP,' ',FP,' ',FN,' ',TN)
        #print(j,' FN ',ModPredicted[j],' ',RawTrue[j])
    #print(TopN,' ',TP,' ',FP,' ',FN,' ',TN)
  ...
  Accuracy= accuracy_score(TrueLableList, PredLableList)*100
  ...
  datadict ={"MicroF1Score":MicroF1Score,
               ...
               "TotalAvgUsersAUC":TotalAvgUsersAUC,
               "AUC":allauc,
               "Accuracy":Accuracy}
...
def inferenceDense(phase,input_batch,relflag, embedding_size=5, device="/cpu:0"):
    ...
    with tf.device(device):
      ...
      train_step=tf.train.GradientDescentOptimizer(0.075).minimize(cost)
...
def svd():
    ...
    with tf.Session() as sess:
        sess.run(init_op)
        ObsCount1=25571 #GetAuxTrainObs(AuxDict)
        ...
        for i in range(EPOCH_MAX):
          ...
          if evaluate:
                ...
                Accuracy=CorrectEval['Accuracy']

                print('Test metrics :',i,' Micro=  ',MicroF1Score,' Macro=  ',MacroF1Score, ' Accuracy= ',Accuracy)
          ##############################
          ...
          if evaluateTrain:
                ...
                print('Train metrics :',i,' Micro=  ',MicroF1Score,' Macro=  ',MacroF1Score, ' Accuracy= ',Accuracy)
...
save_data(MainDict,"../Data/MainDict1.dat")
save_data(AuxDict,"../Data/AuxDict1.dat")
save_data(TRAINNodes,"../Data/TRAINNodes1.dat")
save_data(TESTNodes,"../Data/TESTNodes1.dat")
'''
##



MainDict=load_data("../Data/MainDict1.dat")
AuxDict=load_data("../Data/AuxDict1.dat")
TRAINNodes=load_data("../Data/TRAINNodes1.dat")
TESTNodes=load_data("../Data/TESTNodes1.dat")
